...
export type ChatMessage = { id: string; role: "user" | "assistant"; content: string; attachments?: Attachment[]; sources?: Source[]; executions?: Execution[]; artifacts?: Artifact[]; searchHtml?: string; warning?: string };
export type Chat = { id: string; title: string; messages: ChatMessage[]; updatedAt: number; pinned?: boolean };
...
export function parseWorkspace(value: unknown): Workspace {
  ...
  const chats = data.chats.map((chat) => {
    ...
    return { id: chat.id, title: chat.title.slice(0, 100), updatedAt: chat.updatedAt, pinned: chat.pinned === true, messages: chat.messages.map(cleanMessage) };
  });
...
export function mergeWorkspace(current: Workspace, incoming: Workspace): Workspace {
  ...
  const chats = [...incoming.chats.map((chat) => ({ ...chat, id: makeId(), title: `${chat.title} (importiert)` })), ...current.chats];
  const projects = [...incoming.projects.map((project) => ({ ...project, id: makeId(), name: `${project.name} (importiert)` })), ...current.projects];
