...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { apiKey?: unknown; messages?: Message[] };
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o",
        messages: [
          { role: "system", content: "Du bist ein hilfreicher, präziser Assistent. Antworte standardmäßig auf Deutsch, klar gegliedert und ohne unnötige Wiederholungen." },
          ...messages,
        ],
        temperature: 0.7,
        max_tokens: 1600,
