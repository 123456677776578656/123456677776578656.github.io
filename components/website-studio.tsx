...
import { Code2, Download, History, LoaderCircle, MessageSquare, Monitor, Save, Smartphone, Sparkles, Square, Upload } from "lucide-react";
import { downloadText } from "../lib/files";
import { completeWebsite, websitePreview } from "../lib/website";
import type { Project, Settings } from "../lib/workspace";

export default function WebsiteStudio({ project, onChange, onChat, onBusy, onNeedKey, configured, apiKey, memories, settings }: { project: Project; onChange: (p: Project) => void; onChat: () => void; onBusy: (busy: boolean) => void; onNeedKey: () => void; configured: boolean; apiKey: string; memories: string[]; settings: Settings }) {
  ...
  async function build(event: FormEvent) {
    ...
    try {
      ...
      const result = completeWebsite(data.answer); controller.signal.throwIfAborted();
      setHtml(result); save(result, prompt); setView("preview");
  ...
  const preview = websitePreview(html);
  return <section className="builder-shell website-workspace"><div className="builder-intro"><span className="welcome-kicker">DEINE IDEE. DEINE WEBSEITE.</span><h2>Erstellen. Verfeinern.<br/>Weiterbauen.</h2><p>Beschreibe deine Seite oder ändere ein bestehendes Projekt. Gespeicherte Versionen bleiben zum Wiederherstellen erhalten.</p></div>
    <div className="builder-meta"><label>Projektname<input value={project.name} onChange={(e) => onChange({ ...project, name: e.target.value.slice(0, 80), updatedAt: Date.now() })}/></label><button onClick={() => save()} disabled={!html || loading}><Save size={16}/>Projekt speichern</button><button onClick={onChat} disabled={!html || loading}><MessageSquare size={16}/>Im Chat bearbeiten</button><button onClick={() => importRef.current?.click()} disabled={loading}><Upload size={16}/>HTML importieren</button><input ref={importRef} hidden type="file" accept=".html,.htm" onChange={(e) => void importHtml(e.target.files?.[0])}/></div>
