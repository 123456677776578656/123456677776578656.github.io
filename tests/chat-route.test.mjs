...
const testKey = "test-secret-only-used-in-isolated-tests";

// Run the actual route in isolation: no real credentials or network requests.
function loadRoute(path = "app/api/chat/route.ts", fetch = () => { throw new Error("Unexpected network call"); }) {
  const logs = [];
  function load(filename) {
    ...
    runInNewContext(outputText, {
      ...
      fetch, process: { env: { GEMINI_API_KEY: testKey } },
